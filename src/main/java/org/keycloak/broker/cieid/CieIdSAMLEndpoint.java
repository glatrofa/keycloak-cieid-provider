...
import org.keycloak.saml.processing.core.util.XMLEncryptionUtil.DecryptionKeyLocator;
...
public class CieIdSAMLEndpoint {
    ...
    public CieIdSAMLEndpoint(RealmModel realm, CieIdIdentityProvider provider, CieIdIdentityProviderConfig config, IdentityProvider.AuthenticationCallback callback, DestinationValidator destinationValidator, KeycloakSession session, ClientConnection clientConnection) {
        this.realm = realm;
        this.config = config;
    ...
    protected abstract class Binding {
        ...
        protected Response handleLoginResponse(String samlResponse, SAMLDocumentHolder holder, ResponseType responseType, String relayState, String clientId) {
            ...
            try {
                ...
                String cieIdExpectedRequestId = authSession.getClientNote(SamlProtocol.SAML_REQUEST_ID_BROKER);
                String cieIdResponseValidationError = verifyCieIdResponse(holder.getSamlDocument().getDocumentElement(), assertionElement, cieIdExpectedRequestId);
                ...
                if(AssertionUtil.isIdEncrypted(responseType)) {
                    ...
                    AssertionUtil.decryptId(responseType, (DecryptionKeyLocator) keys.getPrivateKey());
                }
        ...
        private AuthenticationSessionModel samlIdpInitiatedSSO(final String clientUrlName) {
            ...
            Optional<ClientModel> oClient = CieIdSAMLEndpoint.this.session.clients()
            ...
            AuthenticationSessionModel authSession = samlService.getOrCreateLoginSessionForIdpInitiatedSso(session, CieIdSAMLEndpoint.this.realm, oClient.get(), null);
            if (authSession == null) {
