...
public class CieIdIdentityProvider extends AbstractIdentityProvider<CieIdIdentityProviderConfig> {
    ...
    public Object callback(RealmModel realm, AuthenticationCallback callback, EventBuilder event) {
        return new CieIdSAMLEndpoint(realm, this, getConfig(), callback, destinationValidator, session, session.getContext().getConnection());
    }
    ...
    public void authenticationFinished(AuthenticationSessionModel authSession, BrokeredIdentityContext context)  {
        ...
        if (subType != null) {
        	...
        	authSession.setUserSessionNote(CieIdSAMLEndpoint.SAML_FEDERATED_SUBJECT_NAMEID, subjectNameID.serializeAsString());
        }
        AuthnStatementType authn =  (AuthnStatementType)context.getContextData().get(CieIdSAMLEndpoint.SAML_AUTHN_STATEMENT);
        if (authn != null && authn.getSessionIndex() != null) {
            authSession.setUserSessionNote(CieIdSAMLEndpoint.SAML_FEDERATED_SESSION_INDEX, authn.getSessionIndex());
